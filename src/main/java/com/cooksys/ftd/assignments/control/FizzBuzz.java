...
public class FizzBuzz {
    ...
    public static String[] messages(int start, int end) throws IllegalArgumentException {
    	...
    	// Create ArrayList of valid results
    	ArrayList<String> messagesArray = new ArrayList<String>();
    	for (int i = 0; i < arraySize; i++, count++) {
    		messageValue = message(count);
    		if (messageValue != null) {
        		messagesArray.add(messageValue);
    		}
    	}
    	
    	// Insert ArrayList values into String array
    	String[] returnArray = new String[messagesArray.size()];
    	for (int i = 0; i < messagesArray.size(); i++) {
    		returnArray[i] = messagesArray.get(i);
    	}
